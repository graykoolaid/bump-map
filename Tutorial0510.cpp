...
ID3D10EffectShaderResourceVariable* m_bumpMapTexture = NULL;
...
void charLoad( char* filename, vector<const wchar_t *> *textures )
{
	...
	tempTexCount = Import( filename, &vertexices, &indexices, &normexices, &texexices, &texNo );
	characterIndex++;
	Object tempO;
	
	for( int i = 0; i < vertexices.size(); i++ )
	{
		temp.Pos	= D3DXVECTOR3( (float)vertexices[i].x, (float)vertexices[i].y, (float)vertexices[i].z );
		temp.Normal	= D3DXVECTOR3( (float)normexices[i].x, (float)normexices[i].y, (float)normexices[i].z );
		temp.Tex    = D3DXVECTOR2( (float)texexices[i].x, (float)texexices[i].y ); 
		temp.texNum = texNo[i];
		
		temp.Tangent = temp.Pos;

		D3DXVECTOR3 x1 = D3DVECTOR( temp.Normal );
		D3DXVECTOR3 x2 = D3DVECTOR( temp.Tangent );
		D3DXVECTOR3 x3;
		D3DXVec3Cross( &x3, &x1, &x2 );


		temp.BiNormal = D3DXVECTOR3( x3.x, x3.y, x3.z );


		tempO.vertices.push_back( temp );
	}

	for( int i = 0; i < indexices.size(); i++ )
	{
		indices_vec.push_back(  (int)indexices[i]  );
	}
...
//	tempO.vertices  = tempO;
	...
	bd.ByteWidth = sizeof( Vertex ) * tempO.vertices.size();
	bd.BindFlags = D3D10_BIND_VERTEX_BUFFER;
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;
	D3D10_SUBRESOURCE_DATA InitData;
	InitData.pSysMem = &tempO.vertices[0];
	...
	characters[characterIndex].numMeshes = tempTexCount;
...
HRESULT InitDevice()
{
	...
	m_bumpMapTexture			= g_pEffect->GetVariableByName("bumpTexture")->AsShaderResource();
...
//	charLoad( "bigoltrolll.fbx", &textures );
	...
	hr = D3DX10CreateShaderResourceViewFromFile( g_pd3dDevice, L"../assets/Textures/CommandoArmor_NM.dds" , NULL, NULL, &tempSRV, NULL );
	m_bumpMapTexture->SetResource( tempSRV );
	/////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Render()
{
	...
	for( UINT p = 0; p < techDesc.Passes; ++p )
	{
		...
		g_pd3dDevice->DrawIndexed(	characters[characterIndex].indices.size(), 0, 0 );
	}
