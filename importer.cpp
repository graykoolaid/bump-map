...
void DisplayContent(KFbxNode* pNode)
{
	...
	if(mesh)
	{
		...
		{
			//** VERTICES **//
			vertices->push_back( D3DXVECTOR3((float)lControlPoints[i][0], (float)lControlPoints[i][1], (float)lControlPoints[i][2]));
			...
			for (int j = 0; j < mesh->GetElementNormalCount(); j++)
			{
				...
				if (leNormals->GetMappingMode() == FbxGeometryElement::eByControlPoint)
				{
					...
					{
						normals->push_back( D3DXVECTOR3( leNormals->GetDirectArray().GetAt(i)[0], 
				...
				for( int lPolyIndex = 0; lPolyIndex < lPolyCount; ++lPolyIndex )
				{
					...
					for( int lVertIndex = 0; lVertIndex < lPolySize; ++lVertIndex )
					{
						if (lPolyIndexCounter < lIndexCount)
						{
							...
							texcoords[0][lControlPointIndex+size] = D3DXVECTOR2( (float)lUVValue[0], 1.0f-(float) lUVValue[1] );
...
int Import(char *name, vector<D3DXVECTOR3> *vec, vector<WORD> *ind, vector<D3DXVECTOR3> *norm, vector<D3DXVECTOR2> *tex, vector<int> *num )
{
